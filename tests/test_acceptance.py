...
def test_test1():
    response = requests.get(
        "http://app:8080/", params={"came_from": "http://example1.com/toto"}, allow_redirects=False
    )
...
def test_test2():
    response = requests.get(
        "http://app:8080/", params={"came_from": "http://example2.com/toto"}, allow_redirects=False
    )
    assert response.status_code == 302, response.text
...
def test_test_other():
    response = requests.get(
        "http://app:8080/", params={"came_from": "http://example3.com/toto"}, allow_redirects=False
    )
...
def test_wrong():
    response = requests.get(
        "http://app:8080/", params={"came_from2": "http://example2.com/toto"}, allow_redirects=False
    )
    assert response.status_code == 400, response.text


def test_error():
    response = requests.get("http://app:8080/", params={"error": "An error."}, allow_redirects=False)
    assert response.status_code == 400, response.text
